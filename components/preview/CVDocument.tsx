...
export const CVDocument: React.FC<{
  ...
  // Avatar styles — FIXED sizes (no scale multiplier)
  ...
  // Font size — FIXED Tailwind classes (stable scrollHeight)
  ...
  // Spacing — FIXED Tailwind classes (stable scrollHeight)
  ...
  const calculateAutoFit = useCallback(() => {
    ...
    if (settings.template === "modern") {
      // For 2-column layout, take the taller column
      const sbH = sidebarRef.current ? sidebarRef.current.scrollHeight : 0;
      const mainH = mainColRef.current ? mainColRef.current.scrollHeight : 0;
      naturalHeight = Math.max(sbH, mainH);
    } else {
      // For single-column layouts, measure the inner stack
      naturalHeight = innerStackRef.current
    ...
    if (naturalHeight > targetHeight) {
      newScale = Math.max(0.3, (targetHeight - 4) / naturalHeight);
      newScale = Number(newScale.toFixed(3));
    }

    // Only update state if scale actually changed (prevents re-render loops)
    if (Math.abs(newScale - scaleRef.current) >= 0.005) {
      scaleRef.current = newScale;
      setAutoFitScale(newScale);
  ...
  }, [cvData, settings, calculateAutoFit]);
            ...
            <div className={itemSpacingClasses}>
              {experiences.map((exp) => (
                <div key={exp.id} className="break-inside-avoid page-break-inside-avoid space-y-0.5">
                  <div className="flex items-baseline justify-between gap-4 flex-wrap">
                    <div className="min-w-0">
                      <span className="font-bold text-[12.5px] break-words" style={{ color: primaryTextColor }}>{exp.position}</span>
                  ...
                  {exp.highlights && exp.highlights.length > 0 && (
                    <ul className="list-disc list-outside ml-4 space-y-0.5 text-xs" style={{ color: bodyTextColor }}>
                      {exp.highlights.map((h, i) => <li key={i} className="leading-snug break-words whitespace-normal">{h}</li>)}
                    </ul>
                  )}
                </div>
              ))}
              ...
              <div className="space-y-2">
                {skillCategories.map((cat) => (
                  <div key={cat.id} className="space-y-1">
                    <h4 className="text-[11px] font-semibold uppercase tracking-tight break-words" style={{ color: sbTextSecondary }}>{cat.name}</h4>
                    <div className="flex flex-wrap gap-1">
                      {cat.skills.map((s, idx) => (
                        <span key={idx} className="px-1.5 py-0.5 rounded text-[10px] font-medium break-words whitespace-normal" style={{ backgroundColor: sbTagBg, color: sbTagText, border: `1px solid ${sbBorderColor}` }}>{s}</span>
                      ))}
                    </div>
                  </div>
                ))}
            ...
            <div className="grid grid-cols-1 gap-1.5 pt-0.5">
              {skillCategories.map((cat) => (
                <div key={cat.id} className="text-xs flex flex-row items-baseline gap-2">
                  <span className="font-semibold shrink-0 w-32 break-words text-[11.5px]" style={{ color: primaryTextColor }}>{cat.name}:</span>
                  <div className="flex flex-wrap gap-1">
                    {cat.skills.map((s, sIdx) => (
                      <span key={sIdx} className="px-1.5 py-0.5 rounded text-[10px] font-medium break-words" style={{ backgroundColor: tagBgColor, color: tagTextColor }}>{s}</span>
                    ))}
                  </div>
                </div>
              ))}
  ...
  // =========================================================================
  // Shared style for the transform-only scaling wrapper
  // =========================================================================
            ...
            <div className="border-b border-black/10 pb-3 break-inside-avoid">
              <div className="flex items-start justify-between gap-4">
                <div className="space-y-1 flex-1 min-w-0">
                  ...
                  <p className="text-sm font-semibold tracking-tight break-words" style={{ color: accentColor }}>{personalInfo.jobTitle || "Titolo Professionale"}</p>
                  <div className="flex flex-wrap items-center gap-x-3.5 gap-y-1 pt-1.5 text-[11px]" style={{ color: secondaryTextColor }}>
                    {personalInfo.email && <a href={`mailto:${personalInfo.email}`} className="inline-flex items-center gap-1 hover:underline transition-colors break-all" style={{ color: secondaryTextColor }}><Mail className="w-3 h-3 opacity-70 shrink-0" /><span>{personalInfo.email}</span></a>}
                ...
                {settings.showAvatar && personalInfo.avatarUrl && (
                  <div className={cn("overflow-hidden shrink-0 border border-black/10 bg-black/5 shadow-2xs", avatarSizeClasses, avatarShapeClasses)}>
  ...
  if (settings.template === "modern") {
    ...
    return (
      <div
        ...
        style={{ width: "210mm", height: "297mm", minWidth: "210mm", maxWidth: "210mm", minHeight: "297mm", maxHeight: "297mm", background: `linear-gradient(to right, ${sidebarBgColor} 33%, ${paperBgColor} 33%)`, color: primaryTextColor }}
      >
      ...
      <div ref={contentRef} className={cn("p-8 sm:p-10", fontSizeClasses)} style={scaleWrapperStyle}>
        <div ref={innerStackRef} className={spacingClasses}>
          <div className="text-center pb-3 border-b-2 break-inside-avoid space-y-1" style={{ borderColor: accentColor }}>
            {settings.showAvatar && personalInfo.avatarUrl && (
              <div className={cn("overflow-hidden mx-auto mb-1.5 border border-black/15 shadow-2xs", avatarSizeClasses, avatarShapeClasses)}>
                <img src={personalInfo.avatarUrl} alt={personalInfo.fullName} className="w-full h-full object-cover" />
              </div>
            )}
            <h1 className="text-2xl font-serif tracking-tight font-bold break-words leading-tight" style={{ color: primaryTextColor }}>{personalInfo.fullName || "Tuo Nome"}</h1>
            <p className="text-xs font-semibold uppercase tracking-widest break-words" style={{ color: accentColor }}>{personalInfo.jobTitle || "Titolo Professionale"}</p>
            <div className="flex flex-wrap justify-center items-center gap-x-3.5 gap-y-0.5 pt-0.5 text-[11px]" style={{ color: secondaryTextColor }}>
              {personalInfo.email && <a href={`mailto:${personalInfo.email}`} className="hover:underline break-all">{personalInfo.email}</a>}
              {personalInfo.phone && <a href={`tel:${personalInfo.phone.replace(/\s+/g, "")}`} className="hover:underline">• {personalInfo.phone}</a>}
              {personalInfo.location && <span>• {personalInfo.location}</span>}
              {personalInfo.website && <a href={formatUrl(personalInfo.website)} target="_blank" rel="noopener noreferrer" className="hover:underline break-all">• {personalInfo.website.replace(/^https?:\/\//, "")}</a>}
              {personalInfo.linkedin && <a href={formatUrl(personalInfo.linkedin)} target="_blank" rel="noopener noreferrer" className="hover:underline break-all">• {personalInfo.linkedin.replace(/^https?:\/\//, "")}</a>}
              {personalInfo.github && <a href={formatUrl(personalInfo.github)} target="_blank" rel="noopener noreferrer" className="hover:underline break-all">• {personalInfo.github.replace(/^https?:\/\//, "")}</a>}
            </div>
          </div>
          {sectionOrder.filter((s) => s.isVisible).map((section) => renderSectionByKey(section.key, false))}
